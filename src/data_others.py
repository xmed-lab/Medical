#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time    : 2023/10/18 14:32
# @Author  : Ana
# @Site    : 
# @File    : data_others.py
# @Software: PyCharm

# #Desc: This is for data others
...
from data_others_basic import CutoutDefault, augment_list
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        if sample['dc']==0: # domain label
            domain_code = torch.tensor([[1], [0], [0]])
        elif sample['dc']==1:
            domain_code = torch.tensor([[0], [1], [0]])
        elif sample['dc']==2:
            domain_code = torch.tensor([[0], [0], [1]])
        else:
            domain_code = torch.tensor([[0], [0], [0]])
...
def get_dg_segtransform(dataset):
    if 'optic' in dataset:
        ...
        transform_train_imgs = transforms.Compose([
            Identity(),  ## Locate new policy
            DGRandomScaleCrop(512, scale_range=[0.5, 2]),
            Normalize_dg('vessel'),
            ToTensor('vessel')
        ])
        transform_test_imgs = transforms.Compose([
            # DGRandomCrop(256), # 原来是注释掉的
            # Identity(),
            Normalize_dg('vessel'),
            ToTensor('vessel'),
            GenerateMask('vessel')
