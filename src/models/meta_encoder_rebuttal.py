...
# #Desc: 这里使用pretrain的VIT进行实验
import collections
from functools import partial
from timm.models.layers import DropPath, to_2tuple, trunc_normal_
from einops import rearrange
from models.resnet import resnet50
from models.assp import ASPP
import torch
...
class Lora(nn.Module):
    ...
    def forward(self, inputs):
        inputs = inputs.reshape(-1, self.m)
        return torch.mm(torch.mm(inputs, self.A), self.B)


class Adapter(nn.Module):
    """可以不使用，设置对应adapter=False, num_domain为1即可"""
...
class ViT_ImageNet(nn.Module):
    def __init__(self, img_size=224, num_frames=8, patch_size=16, in_chans=3, embed_dim=768, depth=12,
        ...
        self.cls_token = nn.Parameter(torch.zeros(1, 1, embed_dim)) # 可学习的位置编码
    ...
    def forward_features(self, x, d=None):
        # B, C, H, W = x.shape
        int_d = int(d)
        # x = self.patch_embed(x)  # (B,HW,D) # 记得改回来
        x = torch.cat(
...
class Encoder(nn.Module):
    def __init__(self, img_size=224, num_frames=8, patch_size=16, in_chans=3, embed_dim=768, depth=12,
        ...
        self.resnet = resnet50(pretrained_model='/home/czhaobo/Medical/src/models/pretrained/resnet50_v1c.pth', norm_layer=nn.BatchNorm2d,
                                  in_channels=3,
                                  bn_eps=1e-5,
                                  bn_momentum=0.1,
        ...
        # # 如果启用288，则使用下面这个公式
        # self.conv_resize = nn.Sequential(nn.Conv2d(768, 768, kernel_size=5, stride=1, bias=False),
        #             nn.BatchNorm2d(768),
        #             nn.ReLU(inplace=True))
        ...
        # self.lora = Lora(3*config['CROP_SIZE']*config['CROP_SIZE'],14*14* 768, 16) # for lora


        self.conv2 = nn.Sequential(nn.Conv2d(768, 512, kernel_size=3, stride=1, padding=1),
                 nn.BatchNorm2d(512))

        # # for doprompt; for prompt feature
        # self.aspp = ASPP(in_channels=embed_dim, atrous_rates=[6, 12, 18]) # out channel =256
        # self.final_conv = nn.Conv2d(256, 8, kernel_size=1)

    # # for doprompt; vit without finetune
    # def forward(self, x, d=None, meta_step_size=None, meta_loss=None, stop_gradient=None):
    #     x_down = F.interpolate(x, size=(224, 224), mode='bilinear', align_corners=False)
    #
    #     x_trans = self.vit.forward_features(x_down, d) # 16n
    #
    #     # print(x_trans.shape, x_lora.shape)
    #     x_trans = x_trans
    #
    #     x = rearrange(x_trans, "b (x y) c -> b c x y", x=14, y=14) # B, 768, 14, 14
    #
    #     # decoding
    #     out = self.aspp(x)  # (B,C,H/16,W/16) # B, 256, H, W
    #
    #     # upsample
    #     outs = self.final_conv(out)  # (1,H/16,W/16)# B, 1, 14, 14
    #
    #     # print(out.shape)
    #     # x = self.conv2(x)
    #     x = F.interpolate(outs, size=(288, 288), mode='bilinear', align_corners=False)
    #
    #     return out, x

    # for prod also need to change vit (pademb)
    
    # def mask_forward(self,x_down, d=None):
    #     x_trans = self.vit.forward_features(x_down, d) # 16n
    #     # print(x_trans.shape, x_lora.shape)
    #     x = rearrange(x_trans, "b (x y) c -> b c x y", x=14, y=14) # B, 768, 14, 14
    #     # decoding
    ...
    #
    # def forward(self, x, d=None, meta_step_size=None, meta_loss=None, stop_gradient=None):
    #     x1, x2, x3, x4 = self.resnet(x, meta_loss, meta_step_size, stop_gradient) # 记得改回来vit
    #     x_down = self.conv_down(x4) # 768 (这里是参数量的大头似乎)
    #     x_down = F.interpolate(x_down, size=(14, 14), mode='bilinear', align_corners=False)
    #     x_down = x_down.flatten(2).transpose(1, 2)
    #     x = self.mask_forward(x_down,'0')
    #
    #     return x_down, x

    # for transunet & ours;
    def forward(self, x, d=None, meta_step_size=None, meta_loss=None, stop_gradient=None):
        # x_lora = self.lora(x).reshape(x.shape[0],196,768)
        x1, x2, x3, x4 = self.resnet(x, meta_loss, meta_step_size, stop_gradient) # 记得改回来vit
        x_down = self.conv_down(x4) # 768 (这里是参数量的大头似乎)
        x_down = F.interpolate(x_down, size=(14, 14), mode='bilinear', align_corners=False)
        x_down = x_down.flatten(2).transpose(1, 2)
...
if __name__ == "__main__":
    ...
    # print(preds[0].shape) # 这个是最后一层的意思么？
