...
if __name__ == '__main__':
    ...
    output = next(label_dataloader)
    img = output['img']
    mask = output['mask']
    domain_label = output['domain_label']
    mask_params = output['mask_params']
    print(output.keys()) # dict_keys(['img', 'aug_img', 'mask', 'path_img', 'domain_label', 'mask_params'])
    print(img.shape) # B,1,288,288
    print(mask.shape) # B,4,288,288
    print(domain_label.shape) # B,3
    print(mask_params.shape) # B,1,288,288
